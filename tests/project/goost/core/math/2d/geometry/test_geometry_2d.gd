...
var poly_boundary = GeometryExtension2D.regular_polygon(8, SIZE * 2)
var poly_hole = GeometryExtension2D.regular_polygon(4, SIZE)
...
func test_merge_polygons():
	solution = GeometryExtension2D.merge_polygons(poly_a, poly_b)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 8)


func test_clip_polygons():
	solution = GeometryExtension2D.clip_polygons(poly_a, poly_b)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 6)


func test_intersect_polygons():
	solution = GeometryExtension2D.intersect_polygons(poly_a, poly_b)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 4)


func test_exclude_polygons():
	solution = GeometryExtension2D.exclude_polygons(poly_a, poly_b)
...
func test_merge_multiple_polygons():
	solution = GeometryExtension2D.merge_multiple_polygons([poly_a, poly_b, poly_c, poly_d])
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 16)


func test_clip_multiple_polygons():
	solution = GeometryExtension2D.clip_multiple_polygons([poly_a, poly_b], [poly_c, poly_d])
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 10)


func test_intersect_multiple_polygons():
	solution = GeometryExtension2D.intersect_multiple_polygons([poly_a, poly_c], [poly_b, poly_d])
...
func test_exclude_multiple_polygons():
	solution = GeometryExtension2D.exclude_multiple_polygons([poly_a, poly_b], [poly_c, poly_d])
	assert_eq(solution.size(), 2)
	assert_eq(solution[0].size(), 10)
...
func test_polygons_boolean():
	solution = GeometryExtension2D.polygons_boolean(GeometryExtension2D.OPERATION_UNION, [poly_a, poly_b, poly_c, poly_d])
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 16)


func test_polygons_boolean_tree():
	var a = GeometryExtension2D.regular_polygon(4, 150)
	var b = GeometryExtension2D.regular_polygon(4, 100)
	var clip = GeometryExtension2D.clip_polygons(a, b)
	var c = GeometryExtension2D.regular_polygon(4, 50)
	solution = GeometryExtension2D.polygons_boolean_tree(GeometryExtension2D.OPERATION_UNION, clip, [c])
...
func test_clip_polyline_with_polygon():
	solution = GeometryExtension2D.clip_polyline_with_polygon(poly_a, poly_b)
	assert_eq(solution.size(), 2)
...
func test_intersect_polyline_with_polygon():
	solution = GeometryExtension2D.intersect_polyline_with_polygon(poly_a, poly_b)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 3)


func test_clip_multiple_polylines_with_polygons():
	solution = GeometryExtension2D.clip_multiple_polylines_with_polygons([poly_a, poly_c], [poly_b, poly_d])
...
func test_intersect_multiple_polylines_with_polygons():
	solution = GeometryExtension2D.intersect_multiple_polylines_with_polygons([poly_a, poly_c], [poly_b, poly_d])
	assert_eq(solution.size(), 3)
	assert_eq(solution[0].size(), 3)
...
func test_inflate_polygon():
	solution = GeometryExtension2D.inflate_polygon(poly_a, SIZE / 2.0)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 4)


func test_deflate_polygon():
	solution = GeometryExtension2D.deflate_polygon(poly_a, SIZE / 2.0)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 8)


func test_inflate_multiple_polygons():
	solution = GeometryExtension2D.inflate_multiple_polygons([poly_a, poly_c], SIZE / 2.0)
	assert_eq(solution.size(), 2)
	assert_eq(solution[0].size(), 4)
	assert_eq(solution[1].size(), 4)
...
func test_deflate_multiple_polygons():
	solution = GeometryExtension2D.deflate_multiple_polygons([poly_a, poly_c], SIZE / 2.0)
	assert_eq(solution.size(), 1) # Successfully merged together.
	assert_eq(solution[0].size(), 14)


func test_deflate_polyline():
	solution = GeometryExtension2D.deflate_polyline(poly_a, SIZE / 2.0)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 10)


func test_deflate_multiple_polylines():
	solution = GeometryExtension2D.deflate_multiple_polylines([poly_a, poly_c], SIZE / 2.0)
	assert_eq(solution.size(), 1) # Successfully merged together.
...
func test_offset_polygon():
	...
	solution = GeometryExtension2D.offset_polygon(poly_a, SIZE / 2.0, params)
	assert_eq(solution.size(), 1)
...
func test_offset_multiple_polygons():
	...
	params.join_type = PolyOffsetParameters2D.JOIN_ROUND
	params.end_type = PolyOffsetParameters2D.END_ROUND
	solution = GeometryExtension2D.offset_multiple_polygons([poly_a, poly_c], SIZE / 2.0, params)
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 44)


func test_triangulate_polygon():
	solution = GeometryExtension2D.triangulate_polygon(poly_boundary)
...
func test_triangulate_multiple_polygons():
	solution = GeometryExtension2D.triangulate_multiple_polygons([poly_boundary, poly_hole])
...
func test_decompose_polygon_into_convex():
	solution = GeometryExtension2D.decompose_polygon_into_convex(poly_boundary)
	assert_eq(solution.size(), 1)


func test_decompose_multiple_polygons_into_convex():
	solution = GeometryExtension2D.decompose_multiple_polygons_into_convex([poly_boundary, poly_hole, poly_c])
	assert_eq(solution.size(), 6)
	assert_eq(solution[0].size(), 4)
	assert_eq(solution[1].size(), 4)
	assert_eq(solution[2].size(), 4)
...
func test_decompose_polygons_triangles_opt():
	solution = GeometryExtension2D.decompose_polygons(GeometryExtension2D.DECOMP_TRIANGLES_OPT, [poly_boundary])
	assert_eq(solution.size(), 6)
	assert_eq(solution[0].size(), 3)
	assert_eq(solution[1].size(), 3)
	assert_eq(solution[2].size(), 3)
...
func test_decompose_polygons_triangles_mono():
	...
	solution = GeometryExtension2D.decompose_polygons(GeometryExtension2D.DECOMP_TRIANGLES_MONO, [poly_boundary, poly_hole])
	assert_eq(solution.size(), 12)
	assert_eq(solution[0].size(), 3)
	assert_eq(solution[1].size(), 3)
	assert_eq(solution[2].size(), 3)
...
func test_decompose_polygons_convex_opt():
	solution = GeometryExtension2D.decompose_polygons(GeometryExtension2D.DECOMP_CONVEX_OPT, [poly_boundary])
	assert_eq(solution.size(), 1)
	assert_eq(solution[0].size(), 8)


func test_polygon_centroid():
	solution = GeometryExtension2D.polygon_centroid(poly_a)
	assert_eq(solution, Vector2(50, 50))


func test_polygon_perimeter():
	solution = GeometryExtension2D.polygon_perimeter(poly_a)
	assert_eq(solution, 400.0)


func test_polyline_length():
	solution = GeometryExtension2D.polyline_length(poly_a)
	assert_eq(solution, 300.0)


func test_point_in_polygon():
	solution = GeometryExtension2D.point_in_polygon(Vector2(50, 50), poly_a)
	assert_eq(solution, 1) # inside
	solution = GeometryExtension2D.point_in_polygon(Vector2(-50, 50), poly_a)
	assert_eq(solution, 0) # outside
	solution = GeometryExtension2D.point_in_polygon(Vector2(0, 50), poly_a)
	assert_eq(solution, -1) # exactly


func test_regular_polygon():
	solution = GeometryExtension2D.regular_polygon(64, SIZE)
	assert_eq(solution.size(), 64)


func test_circle():
	solution = GeometryExtension2D.circle(SIZE, 0.25)
