...
bool CommandLineParser::_is_options_valid() const {
	...
	for (int i = 0; i < _options.size(); ++i) {
		...
		ERR_FAIL_COND_V_MSG(!default_args.empty() && default_args.size() != option->get_arg_count(), false, vformat("Option '%s' has %d default arguments, but requires %d.", _to_string(option->get_names()), default_args.size(), option->get_arg_count()));
		ERR_FAIL_COND_V_MSG(!default_args.empty() && option->is_required(), false, vformat("Option '%s' cannot have default arguments and be required.", _to_string(option->get_names())));
